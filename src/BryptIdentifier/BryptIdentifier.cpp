...
std::string ConvertToNetworkRepresentation(
    ...
    BryptIdentifier::InternalType const& identifier);
...
std::optional<BryptIdentifier::InternalType> BryptIdentifier::ConvertToInternalRepresentation(
    BryptIdentifier::BufferType const& buffer)
{
    if (buffer.size() != InternalSize) {
        return {};
    }

    BryptIdentifier::InternalType identifier = {};
    ...
    std::string_view identifier)
{
    ...
    if (auto const result = identifier.compare(
        0, BryptIdentifier::Metadata.size(), BryptIdentifier::Metadata); result != 0) {
        return {};
    }

    auto const seperatorPosition = identifier.find_first_of(MetadataSeperator);
    ...
    BufferType identifierBytes(buffer.begin(), buffer.end() - BryptIdentifier::ChecksumSize);
    BufferType receivedChecksumBytes(buffer.end() - BryptIdentifier::ChecksumSize, buffer.end());
    ...
    BufferType generatedChecksumBytes(BryptIdentifier::ChecksumSize, 0x00);
    local::Shake128(upDigestContext.get(), identifierBytes, generatedChecksumBytes, BryptIdentifier::ChecksumSize);
...
std::optional<BryptIdentifier::NetworkType> BryptIdentifier::ConvertToNetworkRepresentation(
    InternalType const& identifier)
{
    local::DigestContext upDigestContext(EVP_MD_CTX_new(), &EVP_MD_CTX_free);
    if (ERR_get_error() != 0 || upDigestContext == nullptr) {
        return {};
...
BryptIdentifier::CContainer::CContainer(InternalType const& identifier)
    : m_internalRepresentation(ReservedIdentifiers::Internal::Invalid)
    , m_networkRepresentation(ReservedIdentifiers::Network::Invalid)
    , m_valid(false)
{
...
BryptIdentifier::NetworkType BryptIdentifier::CContainer::GetNetworkRepresentation() const
{
...
void BryptIdentifier::CContainer::SetupFromInternalRepresentation(InternalType const& identifier)
{
...
{
    BryptIdentifier::BufferType identifier(BryptIdentifier::InternalSize, 0x00);
    do {
        // Get 128 random bits to generate the brypt identifier
        BryptIdentifier::BufferType source(BryptIdentifier::InternalSize, 0x00);
        if (!RAND_bytes(source.data(), BryptIdentifier::InternalSize) || ERR_get_error() != 0) {
            return {};
        }
        ...
        // Brypt network. 
        local::Shake128(upDigestContext.get(), source, identifier, BryptIdentifier::InternalSize);
        if(identifier.empty()) {
            return {};
        }
    ...
    BryptIdentifier::InternalType const& identifier)
{
    ...
    BryptIdentifier::BufferType buffer(BryptIdentifier::InternalSize - bytes.size(), 0x00);
    ...
    BryptIdentifier::BufferType const& identifier)
{
    ...
    BryptIdentifier::BufferType checksum(BryptIdentifier::ChecksumSize, 0x00);
    local::Shake128(upDigestContext.get(), buffer, checksum, BryptIdentifier::ChecksumSize);
    if(checksum.empty()) {
        return {};
    }
    ...
    paylod.append(BryptIdentifier::Metadata);

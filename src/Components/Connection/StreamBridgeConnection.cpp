...
Connection::CStreamBridge::CStreamBridge(
    ...
    , m_socket()
{
    NodeUtils::printo("Creating StreamBridge instance", NodeUtils::PrintType::Connection);
...
{
    NodeUtils::printo("[StreamBridge] I am a StreamBridge implementation", NodeUtils::PrintType::Connection);
}
...
{
    NodeUtils::printo("[StreamBridge] Spawning STREAMBRIDGE connection thread", NodeUtils::PrintType::Connection);
...
void Connection::CStreamBridge::Worker()
{
    ...
    switch (m_operation) {
        case NodeUtils::ConnectionOperation::Server: {
            NodeUtils::printo("[StreamBridge] Setting up StreamBridge socket on port " + m_port, NodeUtils::PrintType::Connection);
    ...
    do {
        ...
        if (optReceivedRaw) {
            ...
            try {
                ...
                NodeUtils::printo("[StreamBridge] Received message failed to unpack.", NodeUtils::PrintType::Connection);
            }
...
void Connection::CStreamBridge::Send(CMessage const& message)
{
    ...
    NodeUtils::printo("[StreamBridge] Sent: (" + std::to_string(strlen(pack.c_str())) + ") " + pack, NodeUtils::PrintType::Connection);
}
...
void Connection::CStreamBridge::Send(std::string_view message)
{
    ...
    NodeUtils::printo("[StreamBridge] Sent: (" + std::to_string(message.size()) + ") " + message.data(), NodeUtils::PrintType::Connection);
}
...
std::optional<std::string> Connection::CStreamBridge::Receive(std::int32_t flag)
{
    ...
    zmq_recv(m_socket.get(), buffer, Connection::StreamBridge::BUFFER_SIZE, flag);
    NodeUtils::printo("[StreamBridge] Received: " + std::string(buffer), NodeUtils::PrintType::Connection);
...
{
    NodeUtils::printo("[StreamBridge] Shutting down socket and context", NodeUtils::PrintType::Connection);

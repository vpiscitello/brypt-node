...
local::Buffer local::GenerateIdentifier(DigestContext const& upDigest)
{
    ...
    do {
        ...
        if (!RAND_bytes(source.data(), Node::Identifier::PayloadBytes)) { return {}; }
...
{
    if (EVP_DigestInit_ex(pDigest, EVP_shake128(), nullptr) != 1) {
        destination.clear();
        return false;
    }

    if (EVP_DigestUpdate(pDigest, source.data(), source.size()) != 1) {
        destination.clear();
        return false;
    }

    if (EVP_DigestFinalXOF(pDigest, destination.data(), length) != 1) {
        destination.clear();
        return false;
    }

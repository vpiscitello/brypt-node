...
std::size_t Message::Application::Parcel::GetPackSize() const
{
    ...
    // Calculate the size of the unencrypted components of the message. 
    std::size_t const unencryptedSize = [this] () -> std::size_t {
        return m_header.GetPackSize() + m_context.GetSignatureSize() + sizeof(std::uint32_t);
    }();

    // Calculate the size for the components of the message that will be encrypted. 
    std::size_t const encryptedSize = [&, this] () -> std::size_t {
        ...
        return m_context.GetEncryptedSize(size); // Update the size to reflect the total encrypted size. 
    }();

    // Determine the size of the message after it is encoded. 
    auto const encoded = Z85::EncodedSize(unencryptedSize + encryptedSize);
...
std::string Message::Application::Parcel::GetPack() const
{
    ...
    // Message Pack Schema:
    // - Section 1 (N bytes): Header
    // - Section 2 (4 bytes): Encrypted Data Size
    // - Section 3 (N bytes): Encrypted Data
    // - Section 4 (N Bytes): Padding
    // - Section 5 (N Bytes): Signature

    // Encrypted Pack Schema: 
    //  - Section 1 (2 bytes): Route Size
    //  - Section 2 (N bytes): Route Data
    //  - Section 3 (4 bytes): Payload Size
    //  - Section 4 (N bytes): Payload Data
    //  - Section 5 (1 byte): Extensions Count
...

    {
        ...
        plaintext.reserve(m_header.GetMessageSize());
        ...
        PackUtils::PackChunk(static_cast<std::uint32_t>(m_context.GetEncryptedSize(plaintext.size())), buffer);

        if (!m_context.Encrypt(plaintext, buffer)) {
            return {}; 
        }
    } 
    ...
    if (!m_context.Sign(buffer)) { return {}; }
...
bool Message::Application::Builder::Unpack(std::span<std::uint8_t const> buffer)
{
    ...
    auto unencryptedBegin = buffer.begin();
    auto const unencryptedEnd = buffer.end();
    if (!m_parcel.m_header.ParseBuffer(unencryptedBegin, unencryptedEnd)) { return false; }
    ...
    auto const headerSize = m_parcel.m_header.GetPackSize();
    std::uint32_t encryptedDataSize = 0;
    if (!PackUtils::UnpackChunk(unencryptedBegin, unencryptedEnd, encryptedDataSize)) { return false; }
    
    auto const optDecryptedBuffer = m_parcel.m_context.Decrypt(Security::ReadableView{ unencryptedBegin, encryptedDataSize });
    if (!optDecryptedBuffer) { return false; }

    std::span decrypted{ *optDecryptedBuffer };
    auto decryptedBegin = decrypted.begin();
    auto const decryptedBnd = decrypted.end();
    {
        std::uint16_t size = 0;
        if (!PackUtils::UnpackChunk(decryptedBegin, decryptedBnd, size)) { return false; }
        if (!PackUtils::UnpackChunk(decryptedBegin, decryptedBnd, m_parcel.m_route, size)) { return false; }
    }
 
    if (!m_parcel.m_payload.Unpack(decryptedBegin, decryptedBnd)) { return false; }

    std::uint8_t extensions = 0;
    if (!PackUtils::UnpackChunk(decryptedBegin, decryptedBnd, extensions)) { return false; }
    if (extensions != 0 && !UnpackExtensions(decryptedBegin, decryptedBnd, extensions)) { return false; }
